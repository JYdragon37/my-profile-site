...
final class MotivationService: ObservableObject {
    ...
    // MARK: - 이미지 프리로드 (Kingfisher 디스크 캐시 — 앱 재시작 시 즉시 표시)
    private func preloadImages(_ images: [MotivationImage]) async {
        ...
        let resources = urls.prefix(5).map { KF.ImageResource(downloadURL: $0) }
        ImagePrefetcher(resources: Array(resources)).start()
    }
