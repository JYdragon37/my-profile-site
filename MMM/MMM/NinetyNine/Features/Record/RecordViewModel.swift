...
// MARK: - Badge Next Progress
struct BadgeNextProgress {
    let badge: Badge
    let current: Int
    let required: Int
    var progress: Double { required > 0 ? min(1.0, Double(current) / Double(required)) : 0 }
    var progressText: String {
        if required <= 1 { return "조건 미달성" }
...
final class RecordViewModel: ObservableObject {
    ...
    // MARK: - 뱃지 다음 목표 진행도
    var nextBadgeProgress: BadgeNextProgress? {
        let badges = BadgeService.shared.badges
        guard let next = badges.first(where: { !$0.isEarned }) else { return nil }
        let (cur, req) = badgeCurrentProgress(for: next.id)
        return BadgeNextProgress(badge: next, current: cur, required: req)
    }

    private func badgeCurrentProgress(for id: String) -> (current: Int, required: Int) {
        let successCount = allRecords.filter { $0.isSuccess }.count
        let fastest = allRecords.filter { $0.isSuccess && $0.elapsedSeconds > 0 }.map { $0.elapsedSeconds }.min() ?? Int.max

        // Streak helpers
        let cal = Calendar.current
        let successDates = allRecords.filter { $0.isSuccess }
            .compactMap { $0.date.toDate() }.map { cal.startOfDay(for: $0) }.sorted()
        var streak = 0, cur = 0
        for i in 1..<successDates.count {
            let diff = cal.dateComponents([.day], from: successDates[max(0,i-1)], to: successDates[i]).day ?? 0
            if diff == 1 { cur += 1; streak = max(streak, cur) } else if diff > 1 { cur = 0 }
        }
        if !successDates.isEmpty { streak = max(streak, 1) }

        // Item counts
        let sparkItems = allRecords.reduce(0) { s, r in
            guard r.itemStatus.count >= 3 else { return s }
            return s + [r.itemStatus[0], r.itemStatus[1], r.itemStatus[2]].filter { $0 }.count
        }
        let flowItems = allRecords.reduce(0) { s, r in
            guard r.itemStatus.count >= 6 else { return s }
            return s + [r.itemStatus[3], r.itemStatus[4], r.itemStatus[5]].filter { $0 }.count
        }
        let deepItems = allRecords.reduce(0) { s, r in
            guard r.itemStatus.count >= 9 else { return s }
            return s + [r.itemStatus[6], r.itemStatus[7], r.itemStatus[8]].filter { $0 }.count
        }

        switch id {
        case "first_challenge":  return (min(successCount, 1), 1)
        case "streak_3":         return (min(streak, 3), 3)
        case "spark_items_15":   return (min(sparkItems, 15), 15)
        case "flow_items_21":    return (min(flowItems, 21), 21)
        case "streak_7":         return (min(streak, 7), 7)
        case "speed_120":        return (fastest <= 120*60 ? 1 : 0, 1)
        case "count_10":         return (min(successCount, 10), 10)
        case "deep_items_30":    return (min(deepItems, 30), 30)
        case "streak_14":        return (min(streak, 14), 14)
        case "speed_90":         return (fastest <= 90*60 ? 1 : 0, 1)
        case "streak_30":        return (min(streak, 30), 30)
        case "count_50":         return (min(successCount, 50), 50)
        default:                 return (0, 1)
        }
    }
